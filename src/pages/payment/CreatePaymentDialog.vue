...
export default {
  ...
  watch: {
    value(val) {
      if (!val) return;
      this.payment = { name: null, methodId: 2, isActive: true };
    }
  },
  methods: {
    async create() {
      ...
      this.run(async () => {
        await this.$store.dispatch('payment/create', this);
        this.$q.notify({ type: 'success', caption: 'Pembayaran berhasil ditambah' });
        this.dialog = false;
      });
    }
  }
