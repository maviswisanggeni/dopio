...
export default {
  ...
  watch: {
    value: function(val) {
      if (!val) return;
      this.purchase = { ...this.purchaseSelected };
      this.panel = 'purchase';
    },
  },
  ...
  methods: {
    ...
    addPurchaseDetail() {
      this.getInventories();
      this.selectedPurchaseDetail = null;
    ...
    async update() {
      ...
      this.run(async () => {
        await this.$store.dispatch('purchase/update', this);
        this.$q.notify({ type: 'success', caption: 'Pembelian berhasil di ubah' });
        this.dialog = false;
      });
    },
    async destroy() {
      this.confirmDeleteDialog(async () => {
        await this.$store.dispatch('purchase/destroy', this);
        this.$q.notify({ type: 'success', caption: 'Pembelian berhasil di hapus' });
        this.dialog = false;
      });
    },
  },
