      ...
      <q-tab-panel name="packet" style="padding: 0px;">
        <base-form ref="form" :onSubmit="update">
          <div class="row items-center justify-center q-pa-xs">
            ...
            <q-space />
            <div class="form-title text-grey-8">Ubah Paket</div>
            <q-space />
            <q-separator vertical class="q-mx-xs" />
...
export default {
  name: 'EditPacketDialog',
  props: { packetSelected: Object },
  ...
  methods: {
    setPacket() {
      this.panel = 'packet';
      this.packet = cloneDeep(this.packetSelected);
      this.filter = '';
    },
    ...
    async update() {
      ...
      this.run(async () => {
        await this.$store.dispatch('packet/update', this);
    ...
    async destroy() {
      this.confirmDeleteDialog(async () => {
        await this.$store.dispatch('packet/destroy', this);
        this.$q.notify({ type: 'success', caption: 'Paket berhasil di hapus' });
        this.dialog = false;
      });
    },
  },
