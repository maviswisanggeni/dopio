      ...
      <q-tab-panel name="extra" style="padding:0px;">
        <base-form ref="form" :onSubmit="create">
          <div class="row items-center justify-center q-pa-xs">
            ...
            <q-space />
            <div class="form-title text-grey-8">Tambah Extra</div>
            <q-space />
            <q-separator vertical class="q-mx-xs" />
...
export default {
  ...
  methods: {
    setExtra() {
      this.panel = 'extra';
      this.extra = { name: null, price: 0, extraIngredients: [] };
      this.filter = '';
    },
    ...
    saveIngredient(extraIngredient) {
      const index = this.extra.extraIngredients.findIndex(ei => ei.ingredientId == extraIngredient.ingredientId);
    ...
    async create() {
      ...
      this.run(async () => {
        await this.$store.dispatch('extra/create', this);
